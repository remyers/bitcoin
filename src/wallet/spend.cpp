        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (utxo_targets_file_path.empty() == false) {
        ...
        coin_selection_params.m_bucket_refill_feerate = CFeeRate(utxo_targets_json["bucket_refill_feerate"].getInt<CAmount>());
    }
    ...
    if (coin_selection_params.m_utxo_targets.size() > 0) {
        ...
        double capacity{1.0};
        auto change_target = GenerateChangeTargetFromUtxoTargets(coin_selection_params.m_utxo_targets, coin_selection_params.m_change_fee, capacity, rng_fast);
        if (change_target) {
            ...
            bool refill = coin_selection_params.m_effective_feerate < coin_selection_params.m_bucket_refill_feerate;
    ...
    if (change_amount > 0) {
        std::optional<std::vector<CTxOut>> change_txouts;
        if (coin_selection_params.m_utxo_targets.size() > 0) {
            change_txouts = SplitChangeFromUtxoTargets(change_amount, coin_selection_params.m_utxo_targets, coin_selection_params.m_min_change_target, coin_selection_params.m_change_fee, rng_fast, scriptChange);
        ...
        if (change_txouts) {
            txNew.vout.insert(txNew.vout.end(), change_txouts->begin(), change_txouts->end());
            // first changeTxOut element is used as our change output
            std::swap(txNew.vout[*change_pos], txNew.vout[txNew.vout.size() - change_txouts->size()]);
        } else {
