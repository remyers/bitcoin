...
bool GenericTransactionSignatureChecker<T>::CheckSchnorrSignature(Span<const unsigned char> sig, const VersionedXOnlyPubKey& verpubkey, SigVersion sigversion, const ScriptExecutionData& execdata, ScriptError* serror) const
{
    ...
    uint256 sighash;
    assert(this->txdata);
    if (!SignatureHashSchnorr(sighash, execdata, *txTo, nIn, hashtype, sigversion, verpubkey.version, *this->txdata)) {
...
static bool VerifyTaprootCommitment(const std::vector<unsigned char>& control, const std::vector<unsigned char>& program, const CScript& script, uint256& tapleaf_hash, std::optional<XOnlyPubKey>* internal_key)
{
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        if (stack.size() == 1) {
            ...
            if (!VerifyTaprootCommitment(control, program, exec_script, execdata.m_tapleaf_hash, &execdata.m_internal_key)) {
                return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
            }
