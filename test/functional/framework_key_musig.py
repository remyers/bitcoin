...
class key_musig(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Repeat test for random participants/hashtype
        for i in range(0,10):

            num_participants = random.randint(3,20)
            ...
            # Determine Segwit output index sent from wallet.
            index = 0
            outputs = tx.vout
            output = outputs[index]
            while (output.scriptPubKey != CScript([OP_1, pk_musig_data_v1])):
                index += 1
                output = outputs[index]
            ...
            sigs = []
            for private_c, public_c in keys_c:
                signature = private_c.sign_musig(nonce_map[public_c], negated, R_agg, pk_musig, sighash)
