...
from .key import ECPubKey, SECP256K1, SECP256K1_FIELD_SIZE, SECP256K1_ORDER, SECP256K1_G, jacobi_symbol
...
def generate_musig_key(pubkey_list):
    ...
    for key in pubkey_list:
        ...
        tweaked_pks.append(key.mul(musig_c[key]))
    return musig_c, reduce(lambda x, y: x + y, tweaked_pks)

def aggregate_schnorr_nonces(nonce_point_list):
    """Construct aggregated musig nonce from individually generated nonces."""
    R_agg = reduce(lambda x, y: x + y, nonce_point_list)
...
def aggregate_musig_signatures(sigs):
    ...
    for idx, sig in enumerate(sigs):
        ...
        R_list.append(R)
    s_agg = reduce(lambda x, y: (x + y) % SECP256K1_ORDER, s_list)
